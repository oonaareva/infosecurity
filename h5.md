...
With this a new encrypted copy of the file will be created, with the file tag .gpg. The original file can still be read with the cat command, so if that is not wnated the original file should be deleted. 
